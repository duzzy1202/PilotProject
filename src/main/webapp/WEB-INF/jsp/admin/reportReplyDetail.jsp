...
<div class="article-detail con">
	...
	<div class="detail-table">
		<div class="detail-box-title">
			<div class="detail-title">${reply.relId}번게시물의 ${reply.id}번 댓글</div>
		</div>
		<div class="detail-box-etc">
			<div class="detail-writer">${reply.extra.writer}</div>
			<div class="detail-regDate">${reply.regDate}</div>
		</div>
		<div class="detail-body">
			<div class="detail-body">${reply.body}</div>
		</div>
		<div class="detail-file">
		<c:set var="file" value="${reply.extra.file__common__attachment['1']}" />
		<c:if test="${file != null}">
			<div>
				<span>첨부된 파일</span> 
				<div> 
		...
		<c:if test="${report.isProcessed == 0}">
			<div class="process-report-box" style="padding: 20px;">
				...
				<form class="process-report flex flex-jc-sa" action="/adm/admin/processReport">
					...
					<input type="hidden" name="reportedType" value="Reply"> 
						<span>댓글 처리</span> 
